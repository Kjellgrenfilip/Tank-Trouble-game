...
void Resource_Manager::get_texture()
{
    ...
    if(!bullet_texture.loadFromFile("resources/textures/bullet.png"))
    {
        throw std::invalid_argument("cannot open bullet_texture");
    }
}
...
sf::Texture& Resource_Manager::get_texture_bullet()
{
    if(bullet_texture.getSize() == sf::Vector2u(0,0))
    {
        get_texture();
    }
